...
description: Create and manage webhooks to receive real-time notifications when jobs and event monitors complete.
---

This guide covers how to create, test, and update a webhook, attach it to a job 
or event monitor, and troubleshoot delivery issues.
...
A webhook is created independently of any job or event monitor. Once created, you
...
Webhook responses include an `auth` object describing the authentication the
...
Use `auth.type` to confirm which scheme is active, and non-secret fields such as
`header` and `username` to verify the rest of the configuration. To rotate a
credential, send a new `auth` object with [Update webhook](#update-webhook) —
you cannot recover the existing secret from the API.

### Attach to a project at creation

Pass `project_id` to associate the webhook with a
[project](/web-search-api/concepts/projects) in the same request:
...
This is equivalent to creating the webhook and then adding it to the project
with `resource_type: webhook`. The project must belong to your organization —
...
## Attach webhook to job or event monitor

You can attach a webhook to a job or event monitor at creation time, or assign it to 
an existing resource afterward.

### At creation time

Pass `webhook_ids` when submitting a job or creating an event monitor:
...
[test calls](#test-before-attaching) apart from production traffic. `test` is
...
on demand without waiting for the next job or event monitor cycle — trigger a webhook
