...
This guide builds a company monitor end to end: upload your list of companies,
run a connected job to confirm the matching, then schedule an event monitor that re-runs
...
<Steps>
  <Step title="Build the watchlist" titleSize="h3">
    ...
    <Tip>
      ...
      [Company Monitors](/web-search-api/concepts/company-monitors) for how to write
  ...
  <Step title="Test a connected job" titleSize="h3">
    ...
    reference it when creating the event monitor.

    <Tip>
      All-news watchlist records carry a `tags` object with `event_type` and
      `sector`, so you can route or filter the feed by what happened. See [Event
      tagging](/web-search-api/concepts/company-monitors#event-tagging).
    </Tip>
    ...
    <Note>
      ...
      [Company Monitors](/web-search-api/concepts/company-monitors#filter-by-association-type).
    </Note>
  </Step>
  ...
  <Step title="Create a webhook" titleSize="h3">
    ...
    jobs or event monitors. Save the returned `webhook.id`.
  ...
  <Step title="Schedule the event monitor" titleSize="h3">
    An event monitor re-runs the connected job on a schedule, deduplicates against previous
    runs, and delivers each run to the webhooks in `webhook_ids`. An event monitor created from
    ...
    <Warning>
      ...
      plan). See [Configure monitors](/web-search-api/how-to/configure-event-monitors).
    </Warning>
  </Step>
...
Portfolios change. Update the dataset without rebuilding the event monitor — the next
...
- [Company Monitors](/web-search-api/concepts/company-monitors): entities, datasets, association types, and per-company scoring in depth
- [Event Monitors](/web-search-api/concepts/event-monitors): how scheduling, deduplication, and rolling date windows work
...
- [Configure event monitors](/web-search-api/how-to/configure-event-monitors): schedules, robust webhook handling, updating an event monitor
