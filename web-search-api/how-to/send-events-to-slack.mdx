...
incoming webhook URL, and attach it to a job or event monitor.
...
<Steps>
  ...
  <Step title="Attach it to a job or event monitor" titleSize="h3">
    Pass the webhook ID in `webhook_ids` when you create an event monitor (for a recurring
    ...
    <Note>
      Need to build the job and event monitor first? See
      [Build a recurring event feed](/web-search-api/how-to/build-an-event-feed) for the
      full query → event monitor → delivery pipeline.
    </Note>
  </Step>
</Steps>
...
- [Build a recurring event feed](/web-search-api/how-to/build-an-event-feed): the full job → event monitor → delivery pipeline
- [Configure event monitors](/web-search-api/how-to/configure-event-monitors): schedules and reliable delivery
