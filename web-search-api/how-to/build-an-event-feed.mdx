...
description: "Go from a one-off query to a scheduled event monitor that delivers fresh event records to your app every day."
---

This guide strings together the full pipeline: test a query, create a webhook,
schedule an event monitor, and handle the results your endpoint receives. Each step links
...
<Steps>
  <Step title="Test the query" titleSize="h3">
    ...
    a schedule. Keep the `job_id`; you reference it when creating the event monitor.
  ...
  <Step title="Schedule an event monitor" titleSize="h3">
    An event monitor re-runs your query on a schedule, deduplicates against previous runs, and
...
create a webhook, schedule an event monitor, handle the delivery. To change which webhooks an
event monitor notifies, `PATCH /catchAll/monitors/{monitor_id}` with new `webhook_ids`.
