---
title: Troubleshoot event monitors
sidebarTitle: Troubleshoot event monitors
description: Diagnose and fix common event monitor issues.
---
...
<Tip>
  ...
  event monitors](/web-search-api/how-to/configure-event-monitors).
</Tip>
...
<Note>
  Event Monitors require `end_date` within 7 days to avoid data gaps. The job's 
...
```
  ...
  <Step title="Check event monitor execution history">
  ...
  <Step title="Adjust query or schedule">
    ...
    **Option 2 - Adjust query scope**: Expand to capture more events without losing business value.
    ...
    Create new reference job → Create new event monitor → Disable old event monitor.
  </Step>
</Steps>

<Tip>
Event monitor deduplication uses a 7-day rolling window. Zero records after the first execution 
...
<Steps>
  ...
  <Step title="Test with webhook.site">
    ...
    your event monitor to verify the issue is with your endpoint.
  </Step>
...
[Configure event monitors: Implement robust webhooks](/web-search-api/how-to/configure-event-monitors#implement-robust-webhooks).
</Tip>
...
2. Review [Event Monitors overview](/web-search-api/concepts/event-monitors) for how event monitors work.
3. Review [Configure event monitors](/web-search-api/how-to/configure-event-monitors) for setup and testing guidance.
