---
title: Company Monitors
sidebarTitle: Company Monitors
description: Track specific companies across CatchAll jobs with per-company relevance scores.
---

Company Monitors filters CatchAll job results to a predefined list of companies
...
Company Monitors uses three connected concepts:
...
portfolio. You create it once and can reuse it across multiple jobs and event monitors.
...
A connected job searches the index restricted to the watchlist entities from the
start, in a single combined pass per query, rather than fetching broadly and
filtering afterwards. You get less noise in the results and faster completion,
with no change to how you submit the job.
...
[Jobs](/web-search-api/concepts/jobs) for the polling pattern.
...
Event Monitors created from an all-news job preserve this behavior — each recurring
run collects all news for the connected watchlist, not topic-restricted results.

## Event tagging

Records from all-news watchlist jobs are classified automatically against a
fixed taxonomy. Each record carries a `tags` object with two fields:

| Field | Type | Notes |
| --- | --- | --- |
| `event_type` | string \| null | Taxonomy path describing what happened, such as `deals.merger_acquisition`. `unclassifiable` when no path fits. |
| `sector` | string \| null | Industry the event belongs to, such as `banking`. `other_sector` when no sector fits. |

Use these to filter, route, or aggregate a watchlist feed downstream — for
example, sending only `deals.*` events to an alerting channel — without parsing
record titles yourself.

<Note>
  Tagging applies only to all-news watchlist jobs, meaning those submitted with
  `fetch_all_watchlist_news: true`. Topic-scoped jobs return no `tags`. The job
  results report `is_all_news_query: true` for jobs that are tagged.
</Note>

Tagging is additive and never filters results. A record the classifier can't
place is still returned, tagged `unclassifiable` or `other_sector` rather than
dropped — so turning to tags can't reduce the events you receive.
...
for record in results.all_records:
    if record.tags:
        print(f"{record.record_title} — {record.tags.event_type} / {record.tags.sector}")
```
...
for (const record of results.all_records) {
  if (record.tags) {
    console.log(`${record.record_title} — ${record.tags.event_type} / ${record.tags.sector}`);
  }
}
```
...
results.getAllRecords().forEach(record ->
    record.getTags().ifPresent(tags ->
        System.out.printf("%s — %s / %s%n",
            record.getRecordTitle(), tags.getEventType(), tags.getSector())
    )
);
```
...
<Expandable title="Tagged record example">

```json
{
  "record_id": "6417909601438475967",
  "record_title": "NVIDIA Acquires Run:ai for $700M",
  "tags": {
    "event_type": "deals.merger_acquisition",
    "sector": "technology"
  },
  "enrichment": {
  ...
  "citations": [
    {
      ...
      "link": "https://example.com/nvidia-runai",
      "published_date": "2026-08-14T09:00:00Z"
    }
  ]
}
```
