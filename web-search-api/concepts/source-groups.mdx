...
applies. When NewsCatcher adds or removes a domain, every later job and event monitor
run picks up the change — you don't need to resubmit anything.

If you want us to add a custom group,
...
Event monitor results report the same information under `reference_job`, so you can
tell which groups a recurring run is scoped to.

## Source groups and event monitors

An event monitor created from a job scoped to source groups keeps that scoping — every
