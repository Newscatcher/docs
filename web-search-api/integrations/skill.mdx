...
from the web — jobs, recurring event monitors, Slack alerts, company monitors, and
...
| File | Purpose |
|---|---|
| `SKILL.md` | Core skill: query construction rules, full MCP tool reference (jobs, event monitors, webhooks, datasets, entities, projects), job modes (`base` / `lite`), limit vs. page_size distinction, watchlist mode end-to-end workflow, full automation sequence, edge case handling |
...
| `references/MONITOR-SCHEDULING.md` | Natural language schedule formats, timezone handling, and webhook configuration examples for recurring event monitors |
