...
submit jobs, poll status, retrieve results, manage event monitors, configure webhooks,
build company monitors with datasets and entities, and organize work into
projects.
...
<Tabs>
  ...
  <Tab title="Event Monitors">
    | Tool | Description |
    |------|-------------|
    | `create_monitor` | Create a recurring event monitor from a completed reference job |
    | `update_monitor` | Update settings (webhook IDs, per-run limit) for an existing event monitor |
    | `list_monitors` | List all event monitors for the authenticated user |
    | `list_monitor_jobs` | List all jobs produced by a specific event monitor |
    | `pull_monitor_results` | Retrieve latest aggregated results from an event monitor |
    | `pull_monitor_csv` | Download the most recent event monitor run results as a CSV file |
    | `get_monitor_status` | Get the full execution history and state changes for an event monitor |
    | `enable_monitor` | Re-enable a previously disabled event monitor |
    | `disable_monitor` | Pause an event monitor without deleting it |
    | `delete_monitor` | Permanently delete an event monitor |
  </Tab>
  ...
  <Tab title="Webhooks">
    ...
    | `list_webhooks` | List all webhook endpoints for the authenticated user — pass an optional `project_id` to filter to webhooks belonging to a specific project |
    ...
    | `assign_webhook_resource` | Attach a webhook to a job or event monitor |
    | `list_webhook_resources` | List all resources (jobs and event monitors) assigned to a webhook |
    | `remove_webhook_resource` | Detach a webhook from a specific resource |
    | `list_resource_webhooks` | List all webhooks attached to a specific job or event monitor |
  ...
  <Tab title="Datasets & Entities">
    ...
    **Entities**
    ...
    | `list_entities` | List all entities for the authenticated user — pass an optional `project_id` to filter to entities belonging to a specific project |
  ...
  <Tab title="Projects">
    Projects group related jobs, event monitors, datasets, and webhooks into named
    ...
    | `add_project_resources` | Add jobs, event monitors, datasets, event monitor groups, or webhooks to a project |
    ...
    | `delete_project` | Delete a project — contained jobs, event monitors, datasets, and event monitor groups are deleted when `delete_resources=true`; webhooks are always only detached, never deleted |
  </Tab>

  <Tab title="Source Groups">
    Source groups are named, reusable domain allowlists — public groups plus any
    organization-visibility groups your organization can access. Use a group's
    `slug` to scope a job's fetching to that domain allowlist.

    | Tool | Description |
    |------|-------------|
    | `list_source_groups` | List all available source groups — returns each group's `slug`, `name`, and `description`; supports optional `page` and `page_size` parameters |
  </Tab>
...
<Accordion title="Tool calls returning errors">
  When a tool call fails, the MCP server reports a real tool error (`isError=True`)
  carrying the upstream status code and message — for example,
  `API Error (401): Api key not found`. Check the error message returned by your
  client for the specific status code and reason.
</Accordion>
